 ...
 *
 * The [getRepository] method is lazily initialized: the zip is only extracted on
...
abstract class ReachabilityMetadataRepositoryService : BuildService<ReachabilityMetadataRepositoryService.Params> {
    ...
    /** Cached repository instance; guarded by [repositoryLock]. */
    @Volatile
    private var cachedRepository: GraalVMReachabilityMetadataRepository? = null
    private val repositoryLock = Any()

    /**
     * Returns the [GraalVMReachabilityMetadataRepository] backed by the unpacked
     * file-system repository.
     *
     * The [zipFile] is supplied by the calling task (keeping it out of the build
     * service parameters avoids configuration-cache serialisation issues with
     ...
     *
     * Thread-safe via a synchronised block.
     */
    fun getRepository(zipFile: File): GraalVMReachabilityMetadataRepository {
        ...
        synchronized(repositoryLock) {
            ...
            return FileSystemRepository(unpackDirFile.toPath()).also { cachedRepository = it }
        }
    }
}
